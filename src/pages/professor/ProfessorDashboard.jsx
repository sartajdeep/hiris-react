  ...
  return (
    <div className="bg-[#F8FAFC] text-[#0F172A] antialiased min-h-screen flex flex-col font-sans overflow-hidden">
      ...
      <main className="flex flex-1 overflow-hidden">
        ...
        {/* CONTENT */}
        <section className="flex-1 flex flex-col overflow-hidden bg-[#F8FAFC]">
          {activeTab === 'dashboard' && (
            <div className="flex-1 flex overflow-hidden animate-[fadeIn_0.2s_ease-out]">
              <div className="flex-1 p-7 overflow-y-auto custom-scrollbar">
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-6">
                    {todayGrid.map(m => (
                      <div key={m.name} className="bg-white border border-[#E8ECF2] shadow-sm rounded-2xl p-5 flex flex-col gap-4 hover:border-[#C8973A]/40 transition-all">
                          <div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                      <div className="bg-white border border-[#E8ECF2] rounded-2xl p-5 flex items-center gap-4 shadow-sm">
                    ...
                    {["09:00 AM","10:00 AM","11:00 AM","12:00 PM","01:00 PM","02:00 PM","03:00 PM"].map(hour => {
                      ...
                      return (
                        <div key={hour} className="relative pl-12 min-h-[64px] border-l border-slate-100">
                          <span className="absolute -left-4 top-0 text-[9px] font-bold text-slate-300 bg-white px-1 uppercase whitespace-nowrap">{hour}</span>
                          {m ? (
          ...
          {activeTab === 'approvals' && (
            <div className="flex-1 flex flex-col p-7 overflow-hidden animate-[fadeIn_0.2s_ease-out]">
              ...
              <div className="flex-1 bg-white border border-[#E8ECF2] shadow-sm rounded-2xl overflow-hidden overflow-y-auto custom-scrollbar">
                  <table className="w-full text-left">
                      <thead className="bg-[#F8FAFC] border-b border-slate-100 sticky top-0">
                  ...
                  <div className="flex items-center gap-2">
                      <div className="flex bg-slate-100 p-1 rounded-xl gap-1">
                          {['All','Approved','Shortlisted','Rejected'].map(s => (
                            <button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 text-[9px] font-bold uppercase rounded-lg transition-all ${statusFilter===s ? 'bg-white shadow-sm text-[#0F1F3D]' : 'text-slate-400'}`}>
                              {s}
                            </button>
                          ))}
                      </div>
                      <input type="text" value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} placeholder="Search…" className="px-3 py-2 bg-white border border-[#E2E8F0] rounded-xl text-xs w-40 focus:border-[#0F1F3D] outline-none" />
                  </div>
              </div>
              <div className="flex-1 bg-white border border-[#E8ECF2] shadow-sm rounded-2xl overflow-hidden overflow-y-auto custom-scrollbar">
                  <table className="w-full text-left">
