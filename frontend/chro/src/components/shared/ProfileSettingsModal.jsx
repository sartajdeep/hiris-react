    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[9999] flex items-center justify-center" onClick={onClose}>
      <div className="bg-[var(--surface)] w-[400px] rounded-xl shadow-2xl overflow-hidden anim-scale-in border border-[var(--border)]" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-[var(--border)] flex justify-between items-center bg-[var(--bg)]">
          <h2 className="font-heading font-bold text-lg text-[var(--navy)] m-0">Profile Settings</h2>
        ...
        <form onSubmit={handleSave} className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input 
                ...
                disabled
                className="w-full px-3 py-2 bg-[var(--border)] border border-[var(--border)] rounded-lg text-sm text-[var(--text-muted)] cursor-not-allowed opacity-70"
              />
            </div>
          </div>
          ...
          <div className="mt-8 flex gap-3">
            <button 
              type="button" 
              onClick={onClose}
              className="flex-1 px-4 py-2 bg-transparent border border-[var(--border)] text-[var(--navy)] rounded-lg font-semibold text-sm hover:bg-[var(--bg)] transition-colors"
            >
              Cancel
            </button>
            <button 
