    ...
    <div className="bg-[#F8FAFC] text-[#0F172A] antialiased flex flex-col min-h-screen font-sans">
      ...
      <main className="flex-1 w-full max-w-[1400px] mx-auto px-8 py-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
          <div>
            <h1 className="text-[22px] font-bold text-[#0F172A] tracking-tight">My Job Requests</h1>
          ...
          <button onClick={() => navigate('/')}
            className="flex items-center gap-1.5 px-4 py-2 border border-[#E2E8F0] rounded-lg text-[12px] font-semibold text-[#475569] hover:bg-white transition shadow-sm">
        ...
        <div className="w-full overflow-x-auto custom-scrollbar pb-4">
          <div className="grid grid-cols-2 gap-5 min-w-[600px] items-start">
            {[
              ...
              return (
                <div key={type} className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    {colReqs.map(r => (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-[10px] text-[#475569]">
      ...
      {selected && (
        <div className="fixed inset-0 z-[998] flex items-center justify-center p-6 bg-[#0F172A]/80 backdrop-blur-sm transition-all duration-300">
          <div className="w-full h-full max-w-6xl bg-white shadow-2xl rounded-2xl flex overflow-hidden animate-[fadeIn_0.3s_ease-out]">
            
            {/* Left Panel: Fully-Rendered JD Preview */}
            <div className="flex-1 overflow-y-auto bg-white border-r border-[#E2E8F0] custom-scrollbar">
              <div className="px-12 py-10">
                {/* Header branding */}
                <div className="flex items-center gap-3 mb-10">
                  <span className="material-symbols-outlined text-[32px] text-[#024e56]">school</span>
                  <div>
                    <h1 className="text-[20px] font-black text-[#0F172A] tracking-tighter leading-none">University Hiring Office</h1>
                ...
                <div className="flex items-start justify-between mb-8">
                  <div>
                    <h2 className="text-[28px] font-extrabold text-[#0F172A] leading-tight tracking-tight">{selected.title}</h2>
                    <div className="flex items-center gap-4 mt-3">
                      <span className="flex items-center gap-1.5 text-[12px] font-bold text-[#475569] uppercase tracking-wider">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-[14px] font-bold text-[#0F172A]">{fmtDeadline(selected.deadline) || 'Open Until Filled'}</p>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  ...
                  <section className="bg-[#F8FAFC] p-6 rounded-xl border border-[#E2E8F0]">
                    <h3 className="text-[12px] font-bold text-[#0F172A] uppercase tracking-widest mb-3">Compensation & Benefits</h3>
            ...
            <div className="w-[380px] shrink-0 bg-[#F8FAFC] flex flex-col">
              {/* Sidebar Header */}
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-6">
                ...
                <div className="flex-1">
                  ...
                  <textarea 
                    ...
                    className="w-full h-[180px] p-4 bg-white border border-[#CBD5E1] rounded-xl text-[13px] text-[#0F172A] focus:outline-none focus:border-[#024e56] focus:ring-2 focus:ring-[#024e56]/10 shadow-sm transition resize-none placeholder:text-[#94A3B8]"
                  />
                    ...
                    {['Needs Revision', 'Expand Scope', 'Clarification Required', 'Approved as-is'].map(f => (
                      <button key={f} onClick={() => setActiveFlag(activeFlag === f ? null : f)}
                        className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border transition-all shadow-sm
                          ${activeFlag === f ? 'bg-[#024e56] text-white border-[#024e56]' : 'bg-white text-[#475569] border-[#CBD5E1] hover:border-[#024e56]/40'}`}>
                        {f}
                      </button>
                    ))}
                  </div>
              ...
              <div className="p-6 bg-white border-t border-[#E2E8F0] space-y-3 shrink-0">
                ...
                <button onClick={() => handleSendBack(selected)} disabled={submitting}
                  className="w-full py-3.5 bg-white border-2 border-[#0F172A] hover:bg-[#0F172A] hover:text-white text-[#0F172A] font-bold rounded-xl transition text-[13px] flex items-center justify-center gap-2 shadow-sm disabled:opacity-60 disabled:hover:bg-white disabled:hover:text-[#0F172A]">
