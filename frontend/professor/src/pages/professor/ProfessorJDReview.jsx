...
export default function ProfessorJDReview() {
  ...
  return (
    <div className="bg-[color:var(--bg)] text-[color:var(--text-primary)] antialiased flex flex-col min-h-screen font-sans">
      ...
      <main className="flex-1 w-full max-w-[1400px] mx-auto px-8 py-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
          <div>
            <h1 className="text-[22px] font-bold text-[color:var(--text-primary)] tracking-tight">My Job Requests</h1>
            <p className="text-[12px] font-medium text-[color:var(--text-secondary)] mt-1">
              All JD requests you have initiated. Click <strong>Awaiting Your Review</strong> to approve or request changes.
            </p>
          </div>
          <button onClick={() => navigate('/')}
            className="flex items-center gap-1.5 px-4 py-2 border border-[color:var(--border)] rounded-lg text-[12px] font-semibold text-[color:var(--text-secondary)] hover:bg-[color:var(--surface)] transition shadow-sm">
        ...
        <div className="w-full overflow-x-auto custom-scrollbar pb-4">
          <div className="grid grid-cols-2 gap-5 min-w-[600px] items-start">
            {[
              ...
              return (
                <div key={type} className="bg-[color:var(--surface)] rounded-xl border border-[color:var(--border)] shadow-sm overflow-hidden">
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    {colReqs.map(r => (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-[10px] text-[color:var(--text-secondary)]">
                          {r.department && r.department !== 'TBD' && (
                            <span className="flex items-center gap-1">
                              <span className="material-symbols-outlined text-sm text-[color:var(--text-muted)]">corporate_fare</span>
                              {r.department}
                            </span>
                          )}
                          {r.location && (
                            <span className="flex items-center gap-1">
                              <span className="material-symbols-outlined text-sm text-[color:var(--text-muted)]">location_on</span>
                              {r.location}
                            </span>
                          )}
                          {r.deadline && (
      ...
      {selected && (
        <div className="fixed inset-0 z-[998] flex items-center justify-center p-6 bg-[#0F172A]/80 backdrop-blur-sm transition-all duration-300">
          <div className="w-full h-full max-w-6xl bg-[color:var(--surface)] shadow-2xl rounded-2xl flex overflow-hidden animate-[fadeIn_0.3s_ease-out]">
            
            {/* Left Panel: Fully-Rendered JD Preview */}
            <div className="flex-1 overflow-y-auto bg-[color:var(--surface)] border-r border-[color:var(--border)] custom-scrollbar">
              <div className="px-12 py-10">
                {/* Header branding */}
                <div className="flex items-center gap-3 mb-10">
                  <span className="material-symbols-outlined text-[32px] text-[#024e56]">school</span>
                  <div>
                    <h1 className="text-[20px] font-black text-[color:var(--text-primary)] tracking-tighter leading-none">University Hiring Office</h1>
                    <p className="text-[11px] font-bold text-[color:var(--text-secondary)] uppercase tracking-[0.15em] mt-1">Official Job Description Preview</p>
                  </div>
                </div>

                <div className="flex items-start justify-between mb-8">
                  <div>
                    <h2 className="text-[28px] font-extrabold text-[color:var(--text-primary)] leading-tight tracking-tight">{selected.title}</h2>
                    <div className="flex items-center gap-4 mt-3">
                      <span className="flex items-center gap-1.5 text-[12px] font-bold text-[color:var(--text-secondary)] uppercase tracking-wider">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-[10px] font-bold text-[color:var(--text-muted)] uppercase tracking-[.15em] mb-1">Target Application Deadline</p>
                    <p className="text-[14px] font-bold text-[color:var(--text-primary)]">{fmtDeadline(selected.deadline) || 'Open Until Filled'}</p>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  ...
                  <section>
                    ...
                    <ul className="space-y-3">
                      {[
                        ...
                        <li key={idx} className="flex gap-3 text-[14px] leading-relaxed text-[color:var(--text-secondary)]">
                  ...
                  <section className="bg-[color:var(--bg)] p-6 rounded-xl border border-[color:var(--border)]">
                    <h3 className="text-[12px] font-bold text-[color:var(--text-primary)] uppercase tracking-widest mb-3">Compensation & Benefits</h3>
            ...
            <div className="w-[380px] shrink-0 bg-[color:var(--bg)] flex flex-col">
              {/* Sidebar Header */}
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-6">
                <div>
                  <p className="text-[11px] font-bold text-[color:var(--text-secondary)] uppercase tracking-widest mb-1.5">Action Required</p>
                  <p className="text-[13px] font-medium text-[color:var(--text-primary)] leading-relaxed">
                ...
                <div className="flex-1">
                  ...
                  <textarea 
                    ...
                    className="w-full h-[180px] p-4 bg-[color:var(--surface)] border border-[color:var(--border)] rounded-xl text-[13px] text-[color:var(--text-primary)] focus:outline-none focus:border-[#024e56] focus:ring-2 focus:ring-[#024e56]/10 shadow-sm transition resize-none placeholder:text-[color:var(--text-muted)]"
                  />
                  <div className="flex justify-between mt-1">
                    <span className="text-[10px] text-[color:var(--text-muted)] font-bold">* Required for sending back</span>
                    <span className="text-[10px] text-[color:var(--text-muted)] font-bold">{feedback.length}/1000</span>
                  </div>
                </div>

                <div className="pt-4 border-t border-[color:var(--border)]">
                  <label className="text-[10px] font-bold text-[color:var(--text-secondary)] uppercase tracking-[.15em] block mb-3">Quick Flags</label>
                    ...
                    {['Needs Revision', 'Expand Scope', 'Clarification Required', 'Approved as-is'].map(f => (
                      <button key={f} onClick={() => setActiveFlag(activeFlag === f ? null : f)}
                        className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border transition-all shadow-sm
                          ${activeFlag === f ? 'bg-[#024e56] text-white border-[#024e56]' : 'bg-[color:var(--surface)] text-[color:var(--text-secondary)] border-[color:var(--border)] hover:border-[#024e56]/40'}`}>
                        {f}
                      </button>
                    ))}
                  </div>
              ...
              <div className="p-6 bg-[color:var(--surface)] border-t border-[color:var(--border)] space-y-3 shrink-0">
                ...
                <button onClick={() => handleSendBack(selected)} disabled={submitting}
                  className="w-full py-3.5 bg-[color:var(--surface)] border-2 border-[#0F172A] hover:bg-[#0F172A] hover:text-white text-[color:var(--text-primary)] font-bold rounded-xl transition text-[13px] flex items-center justify-center gap-2 shadow-sm disabled:opacity-60 disabled:hover:bg-[color:var(--surface)] disabled:hover:text-[color:var(--text-primary)]">
