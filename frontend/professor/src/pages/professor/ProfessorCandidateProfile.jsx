...
export default function ProfessorCandidateProfile() {
  ...
  if (!candidate) return (
    <div className="bg-[color:var(--bg)] min-h-screen flex flex-col font-sans">
  ...
  return (
    <div className="bg-[color:var(--bg)] text-[color:var(--text-primary)] antialiased min-h-screen flex flex-col font-sans">
      ...
      <main className="flex-1 flex flex-col w-full max-w-[1400px] mx-auto px-8 py-6 gap-5">
        ...
        {/* ── Hero card ──────────────────────────── */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[color:var(--surface)] p-5 rounded-xl shadow-sm border border-[color:var(--border)]">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-[18px] font-bold text-[color:var(--text-primary)] leading-tight">{candidate.name}</h1>
              <p className="text-[12px] text-[color:var(--text-secondary)] font-medium mt-0.5">{candidate.role_applied}</p>
              <p className="text-[11px] text-[color:var(--text-muted)] font-medium mt-0.5">
                Applied <strong className="text-[color:var(--text-secondary)]">{candidate.applied_date}</strong> · Ref: {candidate.ref_id}
              </p>
            </div>
          </div>
            ...
            {isReview && (
              <>
                <button onClick={() => navigate(-1)}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-[color:var(--border)] text-[color:var(--text-secondary)] font-semibold text-xs hover:bg-[color:var(--bg)] transition">
        ...
        <div className="flex flex-col lg:flex-row gap-5 flex-1 min-h-[600px]">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-1 custom-scrollbar">
            ...
            {/* Personal info */}
            <section className="bg-[color:var(--surface)] p-5 rounded-xl shadow-sm border border-[color:var(--border)] hover:shadow-md transition-shadow">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-5 gap-x-8">
                {[
                  ...
                  <div key={f.label} className="flex flex-col gap-1">
                    <span className="text-[10px] font-bold text-[color:var(--text-muted)] uppercase tracking-wider">{f.label}</span>
                    <span className="text-[color:var(--text-secondary)] text-xs font-semibold">{f.value}</span>
                  </div>
                ))}
              </div>
            </section>
              ...
              {candidate.education_details ? (
                <div className="space-y-3">
                  {(typeof candidate.education_details === 'string' ? JSON.parse(candidate.education_details) : candidate.education_details).map((e, i) => (
                    <div key={i} className="flex gap-4 p-4 rounded-lg bg-[color:var(--bg)] border border-[color:var(--border)]">
                      <div className="bg-[#024e56]/10 flex items-center justify-center p-3 rounded-lg self-start text-[#024e56]">
                        <span className="material-symbols-outlined text-[18px]">school</span>
                      </div>
                      <div>
                        <h4 className="font-semibold text-sm text-[color:var(--text-primary)] leading-tight mb-0.5">{e.inst}</h4>
                        <p className="text-xs text-[#024e56] font-bold">{e.degree}</p>
                        <p className="text-[11px] text-[color:var(--text-secondary)] font-medium mt-0.5">{e.detail}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex gap-4 p-4 rounded-lg bg-[color:var(--bg)] border border-[color:var(--border)]">
                  ...
                  <div>
                    <h4 className="font-semibold text-sm text-[color:var(--text-primary)] leading-tight mb-0.5">Research University</h4>
                    <p className="text-xs text-[#024e56] font-bold">Doctorate / Postdoctoral</p>
                    <p className="text-[11px] text-[color:var(--text-secondary)] font-medium mt-0.5">Advanced Research · Details on file</p>
                  </div>
                </div>
              )}
            </section>
            ...
            {candidate.skills && (
              <section className="bg-[color:var(--surface)] p-5 rounded-xl shadow-sm border border-[color:var(--border)] hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2 mb-5 text-[#024e56]">
                  ...
                  <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Key Skills</h3>
                </div>
            ...
            <section className="bg-[color:var(--surface)] p-5 rounded-xl shadow-sm border border-[color:var(--border)] hover:shadow-md transition-shadow">
              <div className="flex items-center gap-2 mb-5 text-[#024e56]">
                ...
                <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Application Questions</h3>
              </div>
              {candidate.qa_responses ? (
                <div className="space-y-5">
                  {(typeof candidate.qa_responses === 'string' ? JSON.parse(candidate.qa_responses) : candidate.qa_responses).map((qa, i) => (
                    <div key={i}>
                      <h4 className="text-xs font-bold text-[color:var(--text-primary)] mb-2 italic">"{qa.q}"</h4>
                      <div className="p-3.5 rounded-lg bg-[color:var(--bg)] border-l-4 border-[#024e56] text-[color:var(--text-secondary)] text-xs font-medium leading-relaxed">
                        {qa.a}
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-5">
                  {[
                    ...
                    <div key={i}>
                      <h4 className="text-xs font-bold text-[color:var(--text-primary)] mb-2 italic">"{qa.q}"</h4>
                      <div className="p-3.5 rounded-lg bg-[color:var(--bg)] border-l-4 border-[#024e56] text-[color:var(--text-secondary)] text-xs font-medium leading-relaxed">{qa.a}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            {/* AI summary */}
            <section className="bg-[color:var(--bg)] p-5 rounded-xl shadow-sm border border-[#024e56]/15 mb-4">
              ...
              <p className="text-[color:var(--text-secondary)] text-xs font-medium leading-relaxed mb-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={m.label} className="bg-[color:var(--surface)] p-3 rounded-lg border border-[color:var(--border)]">
                    <div className="flex items-center justify-between mb-1.5">
                      <p className="text-[10px] text-[color:var(--text-secondary)] font-bold uppercase">{m.label}</p>
          ...
          <div className="flex-1 flex flex-col bg-[color:var(--surface)] rounded-xl shadow-sm border border-[color:var(--border)] overflow-hidden">
            {/* Tab bar */}
            <div className="px-5 border-b border-[color:var(--border)] flex items-center justify-between bg-[color:var(--bg)] pt-2">
                  ...
                  <button key={t.id} onClick={() => setActiveTab(t.id)}
                    className={`flex items-center gap-1.5 px-2 pb-2 text-xs transition-all
                      ${activeTab === t.id
                        ...
                        : 'border-b-2 border-transparent text-[color:var(--text-secondary)] hover:text-[color:var(--text-secondary)] font-semibold'}`}
                  >
            ...
            <div className="flex-1 bg-[color:var(--surface)] overflow-y-auto p-5 flex justify-center custom-scrollbar">
              <div className="w-full max-w-2xl bg-[color:var(--surface)] shadow-xl min-h-[800px] p-12 flex flex-col gap-5 border border-[color:var(--border)]">
                ...
                {activeTab === 'resume' && (
                  <>
                    <div className="flex flex-col items-center gap-2 text-center border-b border-[color:var(--border)] pb-8">
                      <h1 className="text-2xl font-bold text-[color:var(--text-primary)] uppercase tracking-wider">{candidate.name}</h1>
                      <p className="text-xs font-semibold text-[color:var(--text-secondary)]">{candidate.location} · {candidate.phone}</p>
                      <p className="text-xs text-[color:var(--text-muted)]">{candidate.email}</p>
                    </div>
                    <div className="space-y-6">
                      <div>
                        <h2 className="text-xs font-bold text-[color:var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Professional Summary</h2>
                        <p className="text-xs font-medium text-[color:var(--text-secondary)] leading-relaxed">
                      ...
                      <div>
                        <h2 className="text-xs font-bold text-[color:var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Selected Highlights</h2>
                        {(candidate.resume_highlights
                          ...
                          <p key={i} className="text-xs font-medium text-[color:var(--text-secondary)] leading-relaxed flex items-start gap-2 mb-2">
                ...
                {activeTab === 'cv' && (
                  <>
                    <div className="flex flex-col items-center gap-2 text-center border-b border-[color:var(--border)] pb-8">
                      <h1 className="text-2xl font-bold text-[color:var(--text-primary)] uppercase tracking-wider">Curriculum Vitae</h1>
                      <p className="text-sm font-semibold text-[#024e56]">{candidate.name}</p>
                    </div>
                    <div className="space-y-6 text-xs">
                      <div>
                        <h2 className="font-bold text-[color:var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Academic Appointments</h2>
                        <p className="text-[color:var(--text-secondary)] font-medium leading-relaxed">Researcher, senior academic and institutional roles documented. Full CV available on request.</p>
                      </div>
                      <div>
                        <h2 className="font-bold text-[color:var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Publications</h2>
                        <p className="text-[color:var(--text-secondary)] font-medium leading-relaxed">Peer-reviewed publications, conference proceedings, and technical reports available from candidate on request.</p>
                      </div>
                      <div>
                        <h2 className="font-bold text-[color:var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Research Interests</h2>
                        <p className="text-[color:var(--text-secondary)] font-medium leading-relaxed">Detailed research interest statement and statement of purpose submitted with application package.</p>
                      </div>
                    </div>
                  </>
                )}
                ...
                {activeTab === 'chat' && (
                  <div>
                    <h2 className="text-sm font-bold text-[color:var(--text-primary)] mb-6">AI Pre-Interview Chat Transcript</h2>
                      ...
                      {[
                        ...
                        <div key={i} className={`flex gap-3 ${msg.role === 'candidate' ? 'flex-row-reverse' : ''}`}>
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white shrink-0 font-bold text-[10px]
                            ${msg.role === 'ai' ? 'bg-[#024e56]' : 'bg-[#E2E8F0] text-[color:var(--text-secondary)]'}`}>
                            {msg.role === 'ai'
                              ? <span className="material-symbols-outlined text-[16px]">smart_toy</span>
                              : candidate.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                          </div>
                          <div className="bg-[color:var(--bg)] p-3 rounded-lg border border-[color:var(--border)] text-xs font-medium text-[color:var(--text-secondary)] max-w-[80%] leading-relaxed">
                            {msg.text}
                          </div>
                        </div>
                      ))}
