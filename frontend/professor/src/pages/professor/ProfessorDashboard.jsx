...
function HireModal({ onClose, onSubmit }) {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/25 backdrop-blur-sm">
      <div className="bg-[color:var(--surface)] w-full max-w-lg rounded-xl shadow-2xl overflow-hidden border border-[color:var(--border)]">
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-[10px] font-bold text-[color:var(--text-muted)] uppercase tracking-widest mb-1.5">Position Title *</label>
            <input value={title} onChange={e => setTitle(e.target.value)}
              placeholder='e.g. "Graduate Research Intern – AI Ethics"'
              className="w-full px-3.5 py-2.5 border border-[color:var(--border)] rounded-lg text-[13px] text-[color:var(--text-primary)] focus:border-[#024e56] focus:ring-2 focus:ring-[#024e56]/10 outline-none bg-[color:var(--bg)] transition" />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-[color:var(--text-muted)] uppercase tracking-widest mb-1.5">
            ...
            <textarea value={description} onChange={e => setDesc(e.target.value)} rows={4}
              ...
              className="w-full px-3.5 py-2.5 border border-[color:var(--border)] rounded-lg text-[13px] text-[color:var(--text-primary)] focus:border-[#024e56] focus:ring-2 focus:ring-[#024e56]/10 outline-none bg-[color:var(--bg)] transition resize-none" />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-[color:var(--text-muted)] uppercase tracking-widest mb-2">Work Location</label>
              ...
              {[
                ...
                <button key={o.key} onClick={() => setLocation(o.key)} type="button"
                  className={`flex-1 py-2.5 rounded-lg text-[10px] font-bold border transition flex items-center justify-center gap-1.5
                    ${location === o.key ? 'bg-[#024e56] text-white border-[#024e56] shadow-sm' : 'bg-[color:var(--surface)] text-[color:var(--text-secondary)] border-[color:var(--border)] hover:border-[#024e56]/40'}`}>
        ...
        <div className="px-6 pb-5 flex gap-2">
          ...
          <button onClick={onClose} disabled={loading}
            className="px-5 py-2.5 border border-[color:var(--border)] text-[color:var(--text-secondary)] font-semibold rounded-lg hover:bg-[color:var(--bg)] text-[13px] transition disabled:opacity-50">
            Cancel
          </button>
        </div>
      </div>
...
export default function ProfessorDashboard() {
  ...
  return (
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-[color:var(--bg)] text-[color:var(--text-primary)] antialiased font-sans">
      

      {/* ── Sub-nav tabs ─────────────────────────────────── */}
      <div className="h-[44px] shrink-0 bg-[color:var(--surface)] border-b border-[color:var(--border)] px-8 flex items-center gap-1">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`relative h-[44px] px-4 text-[12px] font-medium border-b-2 flex items-center gap-2 transition-all
              ${tab === t.id
                ...
                : 'text-[color:var(--text-secondary)] border-transparent hover:text-[#024e56] hover:border-slate-200'}`}>
            {t.label}
            {t.badge > 0 && (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-7">
              <div className="max-w-[1100px] mx-auto space-y-6">
                ...
                <div className="flex items-end justify-between gap-4">
                  <div>
                    <p className="text-[11px] font-medium text-[color:var(--text-muted)]">{dateStr}</p>
                    <h1 className="text-[22px] font-bold text-[color:var(--text-primary)] mt-0.5 tracking-tight">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {sentForApproval.length > 0 && (
                      <button onClick={() => setTab('requests')}
                        className="flex items-center gap-2 px-4 py-2 bg-[color:var(--surface)] border border-amber-200 text-amber-700 text-[12px] font-semibold rounded-lg shadow-sm hover:bg-amber-50 transition-all active:scale-[0.97]">
                ...
                <div className="grid grid-cols-4 gap-4">
                  {[
                    ...
                    <div key={k.label} className="bg-[color:var(--surface)] p-5 rounded-xl border border-[color:var(--border)] shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        {k.tag && <span className={`text-[10px] font-bold ${k.tagColor || 'text-[color:var(--text-muted)]'}`}>{k.tag}</span>}
                      </div>
                      <p className="text-[28px] font-bold text-[color:var(--text-primary)] leading-none">{k.value}</p>
                      <p className="text-[10px] font-bold text-[color:var(--text-muted)] uppercase tracking-wider mt-1.5">{k.label}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-5">
                  ...
                  <div className="flex flex-col min-h-0">
                    <div className="flex items-center justify-between mb-3">
                      <h2 className="text-[14px] font-bold text-[color:var(--text-primary)] tracking-tight">Candidates for Review</h2>
                      {forReview.length > 0 && (
                    ...
                    <div className="bg-[color:var(--surface)] rounded-xl border border-[color:var(--border)] shadow-sm overflow-hidden flex-1">
                      {forReview.length === 0 ? (
                        <div className="py-10 text-center">
                          <span className="material-symbols-outlined text-4xl text-[#E2E8F0] block mb-2">done_all</span>
                          <p className="text-[12px] font-semibold text-[color:var(--text-muted)]">No candidates pending review.</p>
                        </div>
                      ) : forReview.slice(0, 3).map(c => (
                        <div key={c.id} onClick={() => navigate(`/candidate/${c.id}`)}
                          className="flex items-center gap-3 px-4 py-3.5 border-b border-[color:var(--border)] last:border-0 hover:bg-[color:var(--bg)] cursor-pointer group transition-all">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[12px] font-semibold text-[color:var(--text-primary)] group-hover:text-[#024e56] transition-colors truncate">{c.name}</p>
                            <p className="text-[10px] text-[color:var(--text-muted)] truncate">{c.role_applied}</p>
                          </div>
                  ...
                  <div className="flex flex-col min-h-0">
                    <div className="flex items-center justify-between mb-3">
                      <h2 className="text-[14px] font-bold text-[color:var(--text-primary)] tracking-tight">Upcoming Interviews</h2>
                      {scheduled.length > 0 && (
                    ...
                    <div className="bg-[color:var(--surface)] rounded-xl border border-[color:var(--border)] shadow-sm overflow-hidden flex-1">
                      {scheduled.length === 0 ? (
                        <div className="py-10 text-center">
                          ...
                          <p className="text-[12px] font-semibold text-[color:var(--text-muted)]">No scheduled interviews.</p>
                        </div>
                        ...
                        return (
                          <div key={c.id}
                            className={`flex items-center gap-3 px-4 py-3.5 border-b border-[color:var(--border)] last:border-0 transition-all
                              ${imminent ? 'bg-amber-50/50' : 'hover:bg-[color:var(--bg)]'}`}>
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-[12px] font-semibold text-[color:var(--text-primary)] truncate">{c.name}</p>
                              <p className={`text-[10px] font-medium ${imminent ? 'text-amber-600' : 'text-[color:var(--text-muted)]'}`}>
          ...
          {tab === 'approvals' && (
            <div className="flex-1 overflow-hidden flex flex-col p-7">
              <div className="shrink-0 mb-5 flex items-end justify-between gap-4">
                <div>
                  <h1 className="text-[20px] font-bold text-[color:var(--text-primary)] tracking-tight">Candidates for Review</h1>
                  <p className="text-[12px] font-medium text-[color:var(--text-secondary)] mt-0.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="text-[12px] font-medium text-[color:var(--text-secondary)] border border-[color:var(--border)] rounded-lg px-3 py-1.5 bg-[color:var(--surface)] focus:border-[#024e56] focus:ring-2 focus:ring-[#024e56]/10 outline-none cursor-pointer transition">
              ...
              <div className="flex-1 overflow-hidden bg-[color:var(--surface)] rounded-xl border border-[color:var(--border)] shadow-sm flex flex-col">
                <div className="overflow-y-auto flex-1">
                  <table className="w-full text-left">
                    <thead className="bg-[color:var(--bg)] border-b border-[color:var(--border)] sticky top-0 z-10">
                      <tr className="text-[10px] font-bold text-[color:var(--text-muted)] uppercase tracking-widest">
                    ...
                    <tbody className="divide-y divide-[#F1F5F9]">
                      {sortedForReview.map(c => (
                        <tr key={c.id} className="hover:bg-[color:var(--bg)] transition-all">
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate(`/candidate/${c.id}`)}>
                              ...
                              <div>
                                <p className="text-[13px] font-semibold text-[color:var(--text-primary)] group-hover:text-[#024e56] transition-colors">{c.name}</p>
                                <StatusPill status={c.status} />
                              </div>
                            </div>
                          </td>
                          <td className="px-5 py-4 text-[12px] text-[color:var(--text-secondary)]">{c.role_applied}</td>
                          <td className="px-5 py-4 text-[12px] text-[color:var(--text-muted)]">{fmtDate(c.applied_date)}</td>
          ...
          {tab === 'requests' && (
            <div className="flex-1 overflow-hidden flex flex-col p-7">
              <div className="shrink-0 mb-5">
                <h1 className="text-[20px] font-bold text-[color:var(--text-primary)] tracking-tight">My Hire Requests</h1>
                <p className="text-[12px] font-medium text-[color:var(--text-secondary)] mt-0.5">
              ...
              <div className="flex-1 overflow-hidden">
                <div className="grid grid-cols-2 gap-5 h-full">
                  {[
                    ...
                    return (
                      <div key={type} className="bg-[color:var(--surface)] rounded-xl border border-[color:var(--border)] shadow-sm overflow-hidden flex flex-col">
                        <div className="px-5 py-3.5 border-b border-[color:var(--border)] flex items-center justify-between shrink-0">
                          <div className="flex items-center gap-2">
                            <span className={`w-2 h-2 rounded-full ${dot}`} />
                            <h2 className="text-[11px] font-bold uppercase tracking-wider text-[color:var(--text-secondary)]">{label}</h2>
                          </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-3 space-y-2">
                          ...
                          {colReqs.map(r => (
                            <div key={r.id}
                              ...
                              className={`p-4 rounded-xl border border-[color:var(--border)] bg-[color:var(--bg)] space-y-3 shadow-sm transition-all group
                                ${actionable ? 'cursor-pointer hover:border-[#024e56] hover:shadow-md' : ''}`}>
                              <div className="flex items-start justify-between gap-2">
                                <h3 className={`text-[13px] font-semibold text-[color:var(--text-primary)] leading-tight ${actionable ? 'group-hover:text-[#024e56] transition-colors' : ''}`}>
                                  {r.title}
                                </h3>
                                <span className={`text-[9px] font-bold px-2 py-0.5 border rounded-full shrink-0 ${badge}`}>{r.id}</span>
                              </div>
                              {r.description && (
                                <p className="text-[11px] text-[color:var(--text-secondary)] italic line-clamp-2">"{r.description}"</p>
                              )}
                              <div className="flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-[color:var(--text-secondary)]">
                                {r.department && r.department !== 'TBD' && (
                                  <span className="flex items-center gap-1">
                                    <span className="material-symbols-outlined text-sm text-[color:var(--text-muted)]">corporate_fare</span>{r.department}
                                  </span>
                                )}
                                {r.deadline && (
                                  <span className="flex items-center gap-1">
          ...
          {tab === 'schedule' && (
            <div className="flex-1 overflow-hidden flex flex-col p-7">
              <div className="shrink-0 mb-5">
                <h1 className="text-[20px] font-bold text-[color:var(--text-primary)] tracking-tight">Interview Schedule</h1>
                <p className="text-[12px] font-medium text-[color:var(--text-secondary)] mt-0.5">
              ...
              <div className="flex-1 overflow-y-auto space-y-3">
                {scheduled.length === 0 ? (
                  <div className="bg-[color:var(--surface)] rounded-xl border border-[color:var(--border)] py-20 text-center">
                    <span className="material-symbols-outlined text-5xl text-[#E2E8F0] block mb-3">event_busy</span>
                    <p className="text-[13px] font-semibold text-[color:var(--text-muted)]">No interviews scheduled.</p>
                  </div>
                ) : sortedSched.map(c => {
                  const mins     = minsUntil(c.interview_at)
                  const imminent = mins !== null && mins >= 0 && mins <= 15
                  return (
                    <div key={c.id}
                      className={`bg-[color:var(--surface)] rounded-xl border px-5 py-4 flex items-center justify-between gap-6 shadow-sm transition-all
                        ${imminent ? 'border-amber-200 bg-amber-50/30' : 'border-[color:var(--border)] hover:shadow-md hover:border-[#024e56]/20'}`}>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-[color:var(--text-primary)] text-[13px]">{c.name}</p>
                          <p className="text-[11px] text-[color:var(--text-secondary)]">{c.role_applied}</p>
                          <p className={`text-[11px] font-medium mt-0.5 flex items-center gap-1.5 ${imminent ? 'text-amber-600' : 'text-[color:var(--text-muted)]'}`}>
        ...
        {tab === 'overview' && (
          <aside className="w-[240px] shrink-0 bg-[color:var(--surface)] border-l border-[color:var(--border)] flex flex-col overflow-hidden">
            <div className="px-5 pt-5 pb-4 border-b border-[color:var(--border)] flex items-center gap-2 shrink-0">
              <span className="material-symbols-outlined text-[#024e56] text-xl">calendar_month</span>
              <h2 className="text-[12px] font-bold text-[color:var(--text-primary)]">Up Next</h2>
            </div>
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
              {scheduled.length === 0 ? (
                ...
                const imminent = mins !== null && mins >= 0 && mins <= 15
                return (
                  <div key={c.id} className={`relative pl-3.5 border-l-2 py-0.5 ${i === 0 ? 'border-[#024e56]' : 'border-[color:var(--border)]'}`}>
                    <p className={`text-[9px] font-bold uppercase tracking-widest ${i === 0 ? 'text-[#024e56]' : 'text-[color:var(--text-muted)]'}`}>
                      {imminent ? `● ${mins} min away` : fmtInterviewShort(c.interview_at)}
                    </p>
                    <h4 className="text-[12px] font-bold text-[color:var(--text-primary)] mt-0.5 leading-tight">{c.name}</h4>
                    <p className="text-[10px] text-[color:var(--text-muted)] truncate">{c.role_applied}</p>
                    {i === 0 && (
                      <Link to={`/interview/${c.id}`}
