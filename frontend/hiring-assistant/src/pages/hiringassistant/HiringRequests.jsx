    ...
    <div className="bg-[color:var(--bg)] text-[color:var(--text-primary)] antialiased flex flex-col min-h-screen">
      ...
      <main className="flex-grow w-full max-w-[1400px] mx-auto px-8 py-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-[color:var(--text-primary)] tracking-tight">Hiring Requests</h1>
            <p className="text-xs font-medium text-[color:var(--text-secondary)] mt-1">
        ...
        <div className="w-full overflow-x-auto custom-scrollbar pb-4">
          <div className="grid grid-cols-3 gap-5 min-w-[1000px] items-start">
            {COLS.map(col => {
              ...
              return (
                <div key={col} className="bg-[color:var(--surface)] rounded-xl border border-[color:var(--border)] shadow-sm overflow-hidden">
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    {colReqs.map(r => (
                      <div
                        ...
                        className={`p-4 rounded-xl border border-[color:var(--border)] bg-[color:var(--bg)] space-y-3 shadow-sm transition-all group ${isClickable ? 'cursor-pointer hover:border-[#28666E] hover:shadow-md' : ''}`}
                      >
                        <div className="flex items-start justify-between gap-2">
                          <h3 className={`text-sm font-semibold text-[color:var(--text-primary)] leading-tight ${isClickable ? 'group-hover:text-[#28666E] transition-colors' : ''}`}>
                            {r.title}
                          </h3>
