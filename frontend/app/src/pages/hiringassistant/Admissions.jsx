  ...
  return (
    <div className="bg-[var(--bg)] text-[var(--text-primary)] antialiased min-h-screen flex flex-col font-sans">
      ...
      <main className="flex-grow flex flex-col items-center px-6 lg:px-20 py-8">
        <div className="w-full max-w-6xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-3">
                <h1 className="text-[var(--text-primary)] text-2xl font-bold font-heading tracking-tight">Admissions Officer</h1>
              ...
              <p className="text-[var(--text-muted)] font-semibold text-xs tracking-wider uppercase mt-1">Job ID: #{openingId}</p>
            </div>
            
            <div className="flex gap-3">
              {isApplicationsStopped ? (
                <span
                  className="flex items-center justify-center gap-2 px-4 py-2 border rounded-xl font-bold text-xs shadow-sm bg-[var(--surface)] border-[var(--border)] text-[#94A3B8] opacity-50 cursor-not-allowed select-none"
                >
                ...
                <Link
                  to="/admissions/edit"
                  className="flex items-center justify-center gap-2 px-4 py-2 border rounded-xl font-bold text-xs shadow-sm transition-all duration-150 bg-[var(--surface)] border-[var(--border)] text-[#334155] hover:bg-[var(--bg)] hover:shadow-md hover:-translate-y-px hover:border-[#CBD5E1]"
                >
                  <span className="material-symbols-outlined text-[16px]">edit</span> Edit Posting
                </Link>
              )}
              <button 
                ...
                className={`flex items-center justify-center gap-2 px-4 py-2 border rounded-xl font-bold text-xs shadow-sm transition-all duration-150 ${isApplicationsStopped ? 'bg-[#F1F5F9] text-[#94A3B8] border-[var(--border)] cursor-not-allowed' : 'bg-[var(--surface)] border-[#CBD5E1] text-[#EF4444] hover:bg-[#FEF2F2] hover:border-[#F87171] hover:shadow-md hover:-translate-y-px'}`}>
          ...
          <div className="grid grid-cols-1 gap-5 mb-8 sm:grid-cols-2">
            <div className="bg-[var(--surface)] p-5 rounded-xl border border-[var(--border)] shadow-card flex items-center justify-between hover:shadow-md hover:-translate-y-px transition-all duration-150 group">
              <div>
                <p className="text-[#64748B] text-[11px] font-bold uppercase tracking-wider group-hover:text-[var(--text-muted)] transition-colors">Total Applicants</p>
                <p className="text-3xl font-black font-heading text-[var(--text-primary)] mt-1.5">{stats.total}</p>
              </div>
            ...
            <div className="bg-[var(--surface)] p-5 rounded-xl border border-[var(--border)] shadow-card flex items-center justify-between hover:shadow-md hover:-translate-y-px transition-all duration-150 group">
              <div>
                <p className="text-[#64748B] text-[11px] font-bold uppercase tracking-wider group-hover:text-[var(--text-muted)] transition-colors">Pending Review</p>
                <p className="text-3xl font-black font-heading text-[var(--text-primary)] mt-1.5">{stats.pending_review}</p>
              </div>
          ...
          <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow-card overflow-hidden hover:shadow-md transition-shadow duration-150">
            <table className="w-full text-left border-collapse whitespace-nowrap">
              ...
              <tbody className="divide-y divide-[#F1F5F9]">
                {loading ? (
                  ...
                  <tr key={candidate.id} className="hover:bg-[var(--bg)] transition-colors group cursor-pointer" onClick={() => navigate(`/candidate-profile?id=${candidate.id}`)}>
                    <td className="px-6 py-4 text-[13px] font-semibold text-[#64748B] font-mono group-hover:text-[var(--text-muted)] transition-colors">{candidate.ref_id}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2.5">
                        <div className="w-2 h-2 rounded-full bg-[#3B82F6] shadow-[0_0_8px_rgba(59,130,246,0.6)]"></div>
                        <span className="text-[14px] font-bold text-[var(--text-primary)]">{candidate.name}</span>
                      </div>
                    </td>
            ...
            <div className="p-4 border-t border-[var(--border)] bg-[var(--bg)] flex justify-between items-center text-xs text-[#64748B] font-medium">
              <span>Showing {candidates.length} applicant{candidates.length !== 1 ? 's' : ''}</span>
              <span className="text-[#94A3B8] italic">All results shown</span>
            </div>
          </div>
        </div>
      </main>
      ...
      <div className={`fixed inset-0 flex items-center justify-center bg-[#0F172A]/40 z-[100] transition-opacity duration-300 backdrop-blur-sm ${showPopup ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div className={`bg-[var(--surface)] rounded-2xl p-6 md:p-8 max-w-sm w-full mx-4 shadow-2xl transform transition-transform duration-300 flex flex-col items-center text-center ${showPopup ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
          ...
          <h3 className="text-xl font-bold font-heading text-[var(--text-primary)] mb-2 tracking-tight">Applications Stopped</h3>
          <p className="text-[var(--text-muted)] text-sm font-medium leading-relaxed">The application link has been disabled and edit posting is now restricted.</p>
        </div>
      </div>
