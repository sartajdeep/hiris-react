...
export default function Dashboard() {
  ...
  return (
    <div className="bg-[var(--bg)] text-[var(--text-primary)] antialiased flex flex-col min-h-screen">
      ...
      <nav className="h-[40px] px-8 flex items-center bg-[var(--surface)] border-b border-[var(--border)] text-[11px] font-medium text-[#94A3B8]">
      ...
      <main className="dashboard-container flex flex-1">
        {/* LEFT: w-2/3 */}
        <aside className="w-2/3 scrollable-content border-r border-[var(--border)] px-8 py-8 flex flex-col gap-6">
          {/* Welcome */}
          <div>
            <p className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wide mb-1">Wednesday, 18 March 2026</p>
            <h1 className="text-2xl font-heading font-bold text-[var(--text-primary)]">Good morning, Sartajdeep</h1>
            <p className="text-xs font-medium text-[var(--text-muted)] mt-1">Here's what's on your plate today.</p>
          </div>
          {/* Quick Nav Cards */}
          <div className="grid grid-cols-2 gap-5">
            <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow-sm p-5 hover:shadow-md hover:-translate-y-px transition-all duration-150 flex flex-col justify-between min-h-[140px]">
              <div className="w-10 h-10 bg-[#28666E]/5 rounded-lg flex items-center justify-center text-[#28666E]">
                <span className="material-symbols-outlined text-[20px]">inbox</span>
              </div>
              <div className="flex items-end justify-between mt-4">
                <div>
                  <h3 className="text-sm font-semibold text-[var(--text-primary)]">Hiring Requests</h3>
                  <p className="text-xs font-medium text-[var(--text-muted)] mt-0.5">{stats.pending_requests} pending approvals</p>
                </div>
                <Link to="/hiring-requests" className="py-2 px-4 text-xs font-semibold rounded-lg border border-[var(--border)] text-[var(--text-primary)] hover:border-[#28666E] hover:text-[#28666E] transition-all duration-150 bg-[var(--surface)]">View →</Link>
              </div>
            </div>
            <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow-sm p-5 hover:shadow-md hover:-translate-y-px transition-all duration-150 flex flex-col justify-between min-h-[140px]">
              <div className="w-10 h-10 bg-[#28666E]/5 rounded-lg flex items-center justify-center text-[#28666E]">
              ...
              <div className="flex items-end justify-between mt-4">
                <div>
                  <h3 className="text-sm font-semibold text-[var(--text-primary)]">Active Postings</h3>
                  <p className="text-xs font-medium text-[var(--text-muted)] mt-0.5">{stats.active_openings} active</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-5">
              ...
              <div className="col-span-2 space-y-3">
                {[
                  ...
                  <Link key={tile.label} to={tile.to} className="flex items-center justify-between bg-[var(--surface)] border border-[var(--border)] rounded-xl px-4 py-3 shadow-sm hover:shadow-md hover:-translate-y-px transition-all duration-150 h-[60px]">
                    <div>
              ...
              <div className="col-span-3">
                <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm flex flex-col h-[204px]">
                  <div className="flex border-b border-[var(--border)] bg-[var(--bg)] px-1 pt-1 rounded-t-xl">
                    ...
                    <div className="ml-auto px-2 flex items-center">
                      <button onClick={openModal} className="text-[10px] font-bold text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[#F1F5F9] px-2 py-1.5 rounded-lg flex items-center gap-1 transition-colors duration-150">
        ...
        {/* RIGHT: w-1/3 */}
        <section className="w-1/3 bg-[var(--surface)] px-8 py-8 relative border-l border-[var(--border)] shadow-sm z-10 scrollable-content">
