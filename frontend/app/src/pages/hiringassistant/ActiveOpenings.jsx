      ...
      <main className="flex-1 px-4 md:px-10 lg:px-40 py-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div className="flex flex-col gap-1">
            <h1 className="text-[var(--text-primary)] text-2xl font-bold font-heading tracking-tight">Active Postings</h1>
            <p className="text-[var(--text-muted)] text-sm">Manage faculty recruitment and staff acquisition cycles.</p>
          </div>
        </div>

        <div className="mb-8 flex items-center justify-between border-b border-[var(--border)]">
        ...
        <div id="job-container" className={view === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5" : "flex flex-col gap-5"}>
          {loading ? (
            ...
            <div key={job.id} className="job-card flex flex-col bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden shadow-card hover:shadow-md hover:-translate-y-px transition-all duration-150">
              <div className="card-top p-5 border-b border-[var(--border)] bg-[var(--bg)]">
                ...
                <h3 className="text-[var(--text-primary)] text-sm font-semibold leading-tight">{job.title}</h3>
                <p className="text-[var(--text-muted)] text-xs font-medium mt-1">{job.department}</p>
              </div>
              
              <div className="card-bottom p-5 flex flex-col gap-3">
                <div className="flex justify-between items-center">
                  <span className="text-[#94A3B8] text-[10px] font-bold uppercase tracking-wider">Job ID</span>
                  <span className="text-[var(--text-primary)] text-xs font-mono font-semibold">#{job.id}</span>
                </div>
                
                <div className="flex justify-between items-center py-3 border-t border-[var(--border)] mt-1">
                  <div className="flex items-center gap-5">
                    <div className="flex flex-col">
                      <span className="text-[var(--text-muted)] text-[10px] font-bold uppercase tracking-wider">Candidates</span>
                      <span className="text-[#28666E] text-2xl font-black font-heading leading-none">{job.candidates}</span>
                    </div>
                    <div className="flex flex-col border-l border-[var(--border)] pl-5">
                      <span className="text-[#94A3B8] text-[10px] font-bold uppercase tracking-wider">Unopened</span>
                      <span className="text-[#EF4444] text-2xl font-black font-heading leading-none">{Math.floor(job.candidates / 3)}</span>
                    </div>
                  </div>
                  <Link to={`/admissions?opening_id=${job.id}`} className="p-2 rounded-lg bg-[var(--bg)] text-[#28666E] border border-[var(--border)] hover:bg-[#28666E] hover:border-[#28666E] hover:text-white transition-all duration-150 flex items-center justify-center">
