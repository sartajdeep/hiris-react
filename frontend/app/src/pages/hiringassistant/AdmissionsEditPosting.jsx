...
export default function AdmissionsEditPosting() {
  ...
  const commitSkill = () => {
    if (newSkillInput.trim()) setSkills(s => [...s, newSkillInput.trim()])
    ...
    <div className="bg-[var(--bg)] text-[var(--text-primary)] antialiased min-h-screen flex flex-col font-sans">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 flex flex-col lg:flex-row gap-5 w-full">
        <section className="w-full space-y-8 pb-20 lg:w-full">
          ...
          <div className="bg-[var(--surface)] rounded-xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)] border border-[var(--border)] overflow-hidden relative hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)] hover:-translate-y-px transition-all duration-150">
          ...
          <div className="bg-[var(--surface)] rounded-xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)] border border-[var(--border)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)] hover:-translate-y-px transition-all duration-150">
            ...
            <form className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <input className="w-full px-4 py-3 bg-[var(--bg)] border-[var(--border)] rounded-xl text-sm focus:ring-[#28666E]/20 transition-all duration-150" type="text" defaultValue="Graduate Research Intern - AI & Ethics" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-[#334155]">Department</label>
                  <select className="w-full px-4 py-3 bg-[var(--bg)] border-[var(--border)] rounded-xl text-sm focus:ring-[#28666E]/20 transition-all duration-150" defaultValue="CS-01">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer px-4 py-2 bg-[var(--bg)] border-2 border-transparent rounded-xl text-[var(--text-muted)] text-sm font-bold hover:border-[var(--border)]">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-1 mb-2 p-1 bg-[var(--bg)] rounded-lg border border-[var(--border)] w-fit">
                  <button className="p-1.5 hover:bg-slate-200 rounded transition-colors text-[var(--text-muted)]" type="button">
                ...
                <textarea className="w-full px-4 py-3 bg-[var(--bg)] border-[var(--border)] rounded-xl text-sm focus:ring-[#28666E]/20 transition-all duration-150" placeholder="Brief overview of the role..." rows="3"></textarea>
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea className="w-full px-4 py-3 bg-[var(--bg)] border-[var(--border)] rounded-xl text-sm focus:ring-[#28666E]/20 transition-all duration-150" placeholder="List key duties..." rows="5"></textarea>
              </div>
                  ...
                  {showSkillInput ? (
                    <span className="flex items-center gap-1">
                      <input
                        ...
                        className="px-2 py-1 border border-[#28666E] rounded-full text-xs font-medium outline-none w-28"
                        style={{ background: 'var(--bg)', color: 'var(--text-primary)' }}
                      />
                      <button type="button" onClick={commitSkill} className="material-symbols-outlined text-sm text-[#28666E] cursor-pointer">check</button>
                      <button type="button" onClick={() => { setShowSkillInput(false); setNewSkillInput('') }} className="material-symbols-outlined text-sm text-[#94A3B8] cursor-pointer">close</button>
                    </span>
                  ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <label className="flex items-center justify-between p-4 bg-[var(--bg)] rounded-xl cursor-default">
                  ...
                  <label className="flex items-center justify-between p-4 bg-[var(--bg)] rounded-xl cursor-pointer">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setShowQInput(true)} className="text-[#28666E] text-xs font-bold flex items-center gap-1 hover:underline">
                    <span className="material-symbols-outlined text-sm">add_circle</span> Add Question
                  </button>
                </div>
                <div className="space-y-3">
                  {questions.map((q, i) => (
                    <div key={q.id} className="p-4 bg-[var(--bg)] rounded-xl border border-[var(--border)]">
                      <p className="text-xs text-[#94A3B8] mb-1 border-transparent">Question {i + 1}</p>
                      <p className="text-sm font-medium mb-3 border-transparent">{q.text}</p>
                      <div className="flex items-center justify-between text-xs font-bold text-[#94A3B8]">
                        ...
                        <button type="button" onClick={() => setQuestions(qs => qs.filter(x => x.id !== q.id))} className="text-red-500 material-symbols-outlined text-sm">delete</button>
                      </div>
                    </div>
                  ))}
                  {showQInput && (
                    <div className="p-4 bg-[var(--bg)] rounded-xl border-2 border-[#28666E] flex gap-2">
                      <input
                        autoFocus
                        value={newQ}
                        onChange={e => setNewQ(e.target.value)}
                        onKeyDown={e => {
                          if (e.key === 'Enter' && newQ.trim()) { setQuestions(qs => [...qs, { id: Date.now(), text: newQ.trim() }]); setNewQ(''); setShowQInput(false) }
                          if (e.key === 'Escape') { setShowQInput(false); setNewQ('') }
                        }}
                        placeholder="Type question and press Enter…"
                        className="flex-1 text-sm outline-none"
                        style={{ background: 'transparent', color: 'var(--text-primary)' }}
                      />
                      <button type="button" onClick={() => { setShowQInput(false); setNewQ('') }} className="material-symbols-outlined text-sm text-[#94A3B8] cursor-pointer">close</button>
                    </div>
                  )}
                </div>
              </div>
