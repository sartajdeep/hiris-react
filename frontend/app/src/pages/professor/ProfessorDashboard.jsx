...
function HireModal({ onClose, onSubmit }) {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/25 backdrop-blur-sm">
      <div className="bg-[var(--surface)] w-full max-w-lg rounded-xl shadow-2xl overflow-hidden border border-[var(--border)]">
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)}
              ...
              className="w-full px-3.5 py-2.5 border border-[var(--border)] rounded-lg text-[13px] text-[var(--text-primary)] focus:border-[#024e56] focus:ring-2 focus:ring-[#024e56]/10 outline-none bg-[var(--bg)] transition" />
          </div>
          <div>
            ...
            <textarea value={description} onChange={e => setDesc(e.target.value)} rows={4}
              ...
              className="w-full px-3.5 py-2.5 border border-[var(--border)] rounded-lg text-[13px] text-[var(--text-primary)] focus:border-[#024e56] focus:ring-2 focus:ring-[#024e56]/10 outline-none bg-[var(--bg)] transition resize-none" />
          </div>
          <div>
              ...
              {[
                ...
                <button key={o.key} onClick={() => setLocation(o.key)} type="button"
                  className={`flex-1 py-2.5 rounded-lg text-[10px] font-bold border transition flex items-center justify-center gap-1.5
                    ${location === o.key ? 'bg-[#024e56] text-white border-[#024e56] shadow-sm' : 'bg-[var(--surface)] text-[var(--text-muted)] border-[var(--border)] hover:border-[#024e56]/40'}`}>
        ...
        <div className="px-6 pb-5 flex gap-2">
          ...
          <button onClick={onClose} disabled={loading}
            className="px-5 py-2.5 border border-[var(--border)] text-[var(--text-muted)] font-semibold rounded-lg hover:bg-[var(--bg)] text-[13px] transition disabled:opacity-50">
            Cancel
          </button>
        </div>
      </div>
...
export default function ProfessorDashboard() {
  ...
  return (
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-[var(--bg)] text-[var(--text-primary)] antialiased font-sans">
      

      {/* ── Sub-nav tabs ─────────────────────────────────── */}
      <div className="h-[44px] shrink-0 bg-[var(--surface)] border-b border-[var(--border)] px-8 flex items-center gap-1">
        {TABS.map(t => (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="max-w-[1100px] mx-auto space-y-6">
                ...
                <div className="flex items-end justify-between gap-4">
                  <div>
                    ...
                    <h1 className="text-[22px] font-bold text-[var(--text-primary)] mt-0.5 tracking-tight">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {sentForApproval.length > 0 && (
                      <button onClick={() => setTab('requests')}
                        className="flex items-center gap-2 px-4 py-2 bg-[var(--surface)] border border-amber-200 text-amber-700 text-[12px] font-semibold rounded-lg shadow-sm hover:bg-amber-50 transition-all active:scale-[0.97]">
                ...
                <div className="grid grid-cols-4 gap-4">
                  {[
                    ...
                    <div key={k.label} className="bg-[var(--surface)] p-5 rounded-xl border border-[var(--border)] shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <p className="text-[28px] font-bold text-[var(--text-primary)] leading-none">{k.value}</p>
                ...
                <div className="grid grid-cols-2 gap-5">
                  ...
                  <div className="flex flex-col min-h-0">
                    <div className="flex items-center justify-between mb-3">
                      <h2 className="text-[14px] font-bold text-[var(--text-primary)] tracking-tight">Candidates for Review</h2>
                      {forReview.length > 0 && (
                    ...
                    <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow-sm overflow-hidden flex-1">
                      {forReview.length === 0 ? (
                        ...
                        <div key={c.id} onClick={() => navigate(`/candidate/${c.id}`)}
                          className="flex items-center gap-3 px-4 py-3.5 border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg)] cursor-pointer group transition-all">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[12px] font-semibold text-[var(--text-primary)] group-hover:text-[#024e56] transition-colors truncate">{c.name}</p>
                  ...
                  <div className="flex flex-col min-h-0">
                    <div className="flex items-center justify-between mb-3">
                      <h2 className="text-[14px] font-bold text-[var(--text-primary)] tracking-tight">Upcoming Interviews</h2>
                      {scheduled.length > 0 && (
                    ...
                    <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow-sm overflow-hidden flex-1">
                      {scheduled.length === 0 ? (
                        ...
                        return (
                          <div key={c.id}
                            className={`flex items-center gap-3 px-4 py-3.5 border-b border-[var(--border)] last:border-0 transition-all
                              ${imminent ? 'bg-amber-50/50' : 'hover:bg-[var(--bg)]'}`}>
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-[12px] font-semibold text-[var(--text-primary)] truncate">{c.name}</p>
          ...
          {tab === 'approvals' && (
            <div className="flex-1 overflow-hidden flex flex-col p-7">
              <div className="shrink-0 mb-5 flex items-end justify-between gap-4">
                <div>
                  <h1 className="text-[20px] font-bold text-[var(--text-primary)] tracking-tight">Candidates for Review</h1>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="text-[12px] font-medium text-[#334155] border border-[var(--border)] rounded-lg px-3 py-1.5 bg-[var(--surface)] focus:border-[#024e56] focus:ring-2 focus:ring-[#024e56]/10 outline-none cursor-pointer transition">
              ...
              <div className="flex-1 overflow-hidden bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow-sm flex flex-col">
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-[#F1F5F9]">
                      {sortedForReview.map(c => (
                        <tr key={c.id} className="hover:bg-[var(--bg)] transition-all">
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate(`/candidate/${c.id}`)}>
                              ...
                              <div>
                                <p className="text-[13px] font-semibold text-[var(--text-primary)] group-hover:text-[#024e56] transition-colors">{c.name}</p>
                                <StatusPill status={c.status} />
                              </div>
                            </div>
                          </td>
                          <td className="px-5 py-4 text-[12px] text-[var(--text-muted)]">{c.role_applied}</td>
          ...
          {tab === 'requests' && (
            <div className="flex-1 overflow-hidden flex flex-col p-7">
              <div className="shrink-0 mb-5">
                <h1 className="text-[20px] font-bold text-[var(--text-primary)] tracking-tight">My Hire Requests</h1>
                <p className="text-[12px] font-medium text-[var(--text-muted)] mt-0.5">
              ...
              <div className="flex-1 overflow-hidden">
                <div className="grid grid-cols-2 gap-5 h-full">
                  {[
                    ...
                    return (
                      <div key={type} className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow-sm overflow-hidden flex flex-col">
          ...
          {tab === 'schedule' && (
            <div className="flex-1 overflow-hidden flex flex-col p-7">
              <div className="shrink-0 mb-5">
                <h1 className="text-[20px] font-bold text-[var(--text-primary)] tracking-tight">Interview Schedule</h1>
                <p className="text-[12px] font-medium text-[var(--text-muted)] mt-0.5">
              ...
              <div className="flex-1 overflow-y-auto space-y-3">
                {scheduled.length === 0 ? (
                  <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] py-20 text-center">
                  ...
                  return (
                    <div key={c.id}
                      className={`bg-[var(--surface)] rounded-xl border px-5 py-4 flex items-center justify-between gap-6 shadow-sm transition-all
                        ${imminent ? 'border-amber-200 bg-amber-50/30' : 'border-[var(--border)] hover:shadow-md hover:border-[#024e56]/20'}`}>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-[var(--text-primary)] text-[13px]">{c.name}</p>
                          <p className="text-[11px] text-[var(--text-muted)]">{c.role_applied}</p>
        ...
        {tab === 'overview' && (
          <aside className="w-[240px] shrink-0 bg-[var(--surface)] border-l border-[var(--border)] flex flex-col overflow-hidden">
            <div className="px-5 pt-5 pb-4 border-b border-[var(--border)] flex items-center gap-2 shrink-0">
              <span className="material-symbols-outlined text-[#024e56] text-xl">calendar_month</span>
              <h2 className="text-[12px] font-bold text-[var(--text-primary)]">Up Next</h2>
            </div>
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
              {scheduled.length === 0 ? (
                ...
                const imminent = mins !== null && mins >= 0 && mins <= 15
                return (
                  <div key={c.id} className={`relative pl-3.5 border-l-2 py-0.5 ${i === 0 ? 'border-[#024e56]' : 'border-[var(--border)]'}`}>
                    ...
                    <h4 className="text-[12px] font-bold text-[var(--text-primary)] mt-0.5 leading-tight">{c.name}</h4>
                    <p className="text-[10px] text-[#94A3B8] truncate">{c.role_applied}</p>
                    {i === 0 && (
                      <Link to={`/interview/${c.id}`}
