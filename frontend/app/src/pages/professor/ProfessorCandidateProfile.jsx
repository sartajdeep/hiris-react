  ...
  return (
    <div className="bg-[var(--bg)] text-[var(--text-primary)] antialiased min-h-screen flex flex-col font-sans">
      ...
      <main className="flex-1 flex flex-col w-full max-w-[1400px] mx-auto px-8 py-6 gap-5">
        ...
        {/* ── Hero card ──────────────────────────── */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[var(--surface)] p-5 rounded-xl shadow-sm border border-[var(--border)]">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-[18px] font-bold text-[var(--text-primary)] leading-tight">{candidate.name}</h1>
              <p className="text-[12px] text-[var(--text-muted)] font-medium mt-0.5">{candidate.role_applied}</p>
              <p className="text-[11px] text-[#94A3B8] font-medium mt-0.5">
                Applied <strong className="text-[var(--text-muted)]">{candidate.applied_date}</strong> · Ref: {candidate.ref_id}
              </p>
            </div>
          </div>
            ...
            {isReview && (
              <>
                <button onClick={() => navigate(-1)}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-[var(--border)] text-[var(--text-muted)] font-semibold text-xs hover:bg-[var(--bg)] transition">
        ...
        <div className="flex flex-col lg:flex-row gap-5 flex-1 min-h-[600px]">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-1 custom-scrollbar">
            ...
            {/* Personal info */}
            <section className="bg-[var(--surface)] p-5 rounded-xl shadow-sm border border-[var(--border)] hover:shadow-md transition-shadow">
              ...
              {candidate.education_details ? (
                <div className="space-y-3">
                  {(typeof candidate.education_details === 'string' ? JSON.parse(candidate.education_details) : candidate.education_details).map((e, i) => (
                    <div key={i} className="flex gap-4 p-4 rounded-lg bg-[var(--bg)] border border-[var(--border)]">
                      <div className="bg-[#024e56]/10 flex items-center justify-center p-3 rounded-lg self-start text-[#024e56]">
                        <span className="material-symbols-outlined text-[18px]">school</span>
                      </div>
                      <div>
                        <h4 className="font-semibold text-sm text-[var(--text-primary)] leading-tight mb-0.5">{e.inst}</h4>
                        <p className="text-xs text-[#024e56] font-bold">{e.degree}</p>
                        <p className="text-[11px] text-[var(--text-muted)] font-medium mt-0.5">{e.detail}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex gap-4 p-4 rounded-lg bg-[var(--bg)] border border-[var(--border)]">
                  ...
                  <div>
                    <h4 className="font-semibold text-sm text-[var(--text-primary)] leading-tight mb-0.5">Research University</h4>
                    <p className="text-xs text-[#024e56] font-bold">Doctorate / Postdoctoral</p>
                    <p className="text-[11px] text-[var(--text-muted)] font-medium mt-0.5">Advanced Research · Details on file</p>
                  </div>
                </div>
              )}
            </section>
            ...
            {candidate.skills && (
              <section className="bg-[var(--surface)] p-5 rounded-xl shadow-sm border border-[var(--border)] hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2 mb-5 text-[#024e56]">
                  ...
                  <h3 className="text-sm font-semibold text-[var(--text-primary)]">Key Skills</h3>
                </div>
            ...
            <section className="bg-[var(--surface)] p-5 rounded-xl shadow-sm border border-[var(--border)] hover:shadow-md transition-shadow">
              <div className="flex items-center gap-2 mb-5 text-[#024e56]">
                ...
                <h3 className="text-sm font-semibold text-[var(--text-primary)]">Application Questions</h3>
              </div>
              {candidate.qa_responses ? (
                <div className="space-y-5">
                  {(typeof candidate.qa_responses === 'string' ? JSON.parse(candidate.qa_responses) : candidate.qa_responses).map((qa, i) => (
                    <div key={i}>
                      <h4 className="text-xs font-bold text-[var(--text-primary)] mb-2 italic">"{qa.q}"</h4>
                      <div className="p-3.5 rounded-lg bg-[var(--bg)] border-l-4 border-[#024e56] text-[var(--text-muted)] text-xs font-medium leading-relaxed">
                        {qa.a}
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-5">
                  {[
                    ...
                    <div key={i}>
                      <h4 className="text-xs font-bold text-[var(--text-primary)] mb-2 italic">"{qa.q}"</h4>
                      <div className="p-3.5 rounded-lg bg-[var(--bg)] border-l-4 border-[#024e56] text-[var(--text-muted)] text-xs font-medium leading-relaxed">{qa.a}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            {/* AI summary */}
            <section className="bg-[var(--bg)] p-5 rounded-xl shadow-sm border border-[#024e56]/15 mb-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={m.label} className="bg-[var(--surface)] p-3 rounded-lg border border-[var(--border)]">
                    <div className="flex items-center justify-between mb-1.5">
                      <p className="text-[10px] text-[var(--text-muted)] font-bold uppercase">{m.label}</p>
          ...
          <div className="flex-1 flex flex-col bg-[var(--surface)] rounded-xl shadow-sm border border-[var(--border)] overflow-hidden">
            {/* Tab bar */}
            <div className="px-5 border-b border-[var(--border)] flex items-center justify-between bg-[var(--bg)] pt-2">
            ...
            <div className="flex-1 bg-[#F1F5F9] overflow-y-auto p-5 flex justify-center custom-scrollbar">
              <div className="w-full max-w-2xl bg-[var(--surface)] shadow-xl min-h-[800px] p-12 flex flex-col gap-5 border border-[var(--border)]">
                ...
                {activeTab === 'resume' && (
                  <>
                    <div className="flex flex-col items-center gap-2 text-center border-b border-[var(--border)] pb-8">
                      <h1 className="text-2xl font-bold text-[var(--text-primary)] uppercase tracking-wider">{candidate.name}</h1>
                    ...
                    <div className="space-y-6">
                      <div>
                        <h2 className="text-xs font-bold text-[var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Professional Summary</h2>
                      ...
                      <div>
                        <h2 className="text-xs font-bold text-[var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Selected Highlights</h2>
                        {(candidate.resume_highlights
                          ...
                          <p key={i} className="text-xs font-medium text-[var(--text-muted)] leading-relaxed flex items-start gap-2 mb-2">
                ...
                {activeTab === 'cv' && (
                  <>
                    <div className="flex flex-col items-center gap-2 text-center border-b border-[var(--border)] pb-8">
                      <h1 className="text-2xl font-bold text-[var(--text-primary)] uppercase tracking-wider">Curriculum Vitae</h1>
                      <p className="text-sm font-semibold text-[#024e56]">{candidate.name}</p>
                    </div>
                    <div className="space-y-6 text-xs">
                      <div>
                        <h2 className="font-bold text-[var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Academic Appointments</h2>
                        <p className="text-[var(--text-muted)] font-medium leading-relaxed">Researcher, senior academic and institutional roles documented. Full CV available on request.</p>
                      </div>
                      <div>
                        <h2 className="font-bold text-[var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Publications</h2>
                        <p className="text-[var(--text-muted)] font-medium leading-relaxed">Peer-reviewed publications, conference proceedings, and technical reports available from candidate on request.</p>
                      </div>
                      <div>
                        <h2 className="font-bold text-[var(--text-primary)] border-b-2 border-[#0F172A] pb-1 w-fit uppercase tracking-wider mb-3">Research Interests</h2>
                        <p className="text-[var(--text-muted)] font-medium leading-relaxed">Detailed research interest statement and statement of purpose submitted with application package.</p>
                      </div>
                    </div>
                  </>
                )}
                ...
                {activeTab === 'chat' && (
                  <div>
                    <h2 className="text-sm font-bold text-[var(--text-primary)] mb-6">AI Pre-Interview Chat Transcript</h2>
                    <div className="space-y-4">
                      {[
                        ...
                        <div key={i} className={`flex gap-3 ${msg.role === 'candidate' ? 'flex-row-reverse' : ''}`}>
                          ...
                          <div className="bg-[var(--bg)] p-3 rounded-lg border border-[var(--border)] text-xs font-medium text-[#334155] max-w-[80%] leading-relaxed">
                            {msg.text}
                          </div>
                        </div>
                      ))}
