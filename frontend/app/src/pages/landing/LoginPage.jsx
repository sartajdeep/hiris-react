...
const DEMO_USERS = {
  ...
  'sartajdeep.singh@hiris.demo':  { name: 'Sartajdeep Singh',  role: 'hiring-assistant', portal: 'Hiring Assistant', initials: 'SS', portalUrl: 'http://localhost:5173', color: '#28666E' },
    ...
    <>
      <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'var(--font-body)' }}>
        ...
        <div style={{ paddingTop: 110, paddingBottom: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
          <div style={{ width: '100%', maxWidth: 420, padding: '0 24px' }}>
            {/* Card */}
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '36px 36px', boxShadow: 'var(--shadow-md)' }}>
              <div style={{ marginBottom: 28 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: 'var(--teal)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
                  <span style={{ color: 'white', fontWeight: 900, fontSize: 18, fontFamily: "'Times New Roman', Times, serif" }}>H</span>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
                {[{ label: 'Sign in with Google', Icon: GoogleIcon, provider: 'Google' }, { label: 'Sign in with Microsoft', Icon: MsIcon, provider: 'Microsoft' }].map(({ label, Icon, provider }) => (
                  <button key={provider} onClick={() => { setSsoEmail(''); setSsoModal(provider) }}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '11px', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--surface)', fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', cursor: 'pointer', transition: 'background 0.15s' }}
            ...
            {/* Demo hint */}
            <div style={{ marginTop: 20, padding: '14px 16px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, fontSize: 12, color: 'var(--text-secondary)' }}>
              <div style={{ fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>Demo accounts</div>
              {Object.entries(DEMO_USERS).map(([email, cfg]) => (
                <button key={email} onClick={() => setEmail(email)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '5px 0', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: 'var(--text-secondary)', transition: 'color 0.1s' }}
      ...
      {ssoModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: 24 }}>
          <div style={{ background: 'var(--surface)', borderRadius: 14, padding: 30, width: '100%', maxWidth: 360, boxShadow: '0 24px 60px rgba(0,0,0,0.2)' }}>
            <h3 style={{ fontSize: 16, marginBottom: 6 }}>Sign in with {ssoModal}</h3>
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 18 }}>Enter your {ssoModal} account email to continue.</p>
...
const labelStyle = { display: 'block', fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }
const inputSt = { width: '100%', padding: '10px 13px', border: '1.5px solid var(--border)', borderRadius: 8, fontSize: 13, background: 'var(--surface)', color: 'var(--text-primary)', outline: 'none', fontFamily: 'var(--font-body)', boxSizing: 'border-box', transition: 'border-color 0.15s' }
