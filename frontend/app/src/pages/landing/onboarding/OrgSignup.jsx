...
import { registerOrganisation } from '../../../api/client'
...
const DEFAULT_ROLES = [
  ...
  {
    key: 'hiring-assistant', label: 'Hiring Assistant', color: '#0F172A',
...
const STEPS = [
  ...
  { n: 4, title: 'Launch',        icon: 'rocket_launch' },
]
  ...
  function handleLaunch() {
    const users = invites
      ...
      .map(i => {
        ...
        const portal = i.roleKey === 'chro' ? 'CHRO Portal' : i.roleKey === 'hiring-assistant' ? 'Hiring Assistant' : 'Faculty Portal'
        const portalUrl = 'http://localhost:5173/login'
        return {
    ...
    // Persist to backend DB (non-blocking — localStorage auth still works if API is down)
    registerOrganisation(org, users).catch(err =>
      console.warn('Could not save org to backend:', err)
    )
        ...
        <div style={{ maxWidth: 740, margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ marginBottom: 36 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>Step {step} of 4</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
              {STEPS.map(s => (
                <div key={s.n} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 12px', borderRadius: 20, fontSize: 11, fontWeight: 700, border: `1px solid ${s.n === step ? 'var(--teal)' : s.n < step ? 'rgba(40,102,110,0.3)' : 'var(--border)'}`, background: s.n === step ? 'rgba(40,102,110,0.1)' : 'transparent', color: s.n === step ? 'var(--teal)' : s.n < step ? 'var(--teal)' : 'var(--text-muted)' }}>
                ...
                {roles.map((role, ri) => (
                  <div key={role.key} style={{ border: '1px solid var(--border)', borderRadius: 12, padding: '18px 20px', background: 'var(--surface)' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 8, background: 'rgba(40,102,110,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <span className="material-symbols-outlined" style={{ fontSize: 18, color: 'var(--teal)' }}>badge</span>
                      </div>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)' }}>{role.label}</div>
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>{role.desc}</div>
                      </div>
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                      {Object.entries(role.perms).map(([key, val]) => (
                        <label key={key} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 10px', borderRadius: 6, border: `1px solid ${val ? 'rgba(40,102,110,0.3)' : 'var(--border)'}`, background: val ? 'rgba(40,102,110,0.1)' : 'transparent', cursor: 'pointer', fontSize: 11, fontWeight: 600, color: val ? 'var(--teal)' : 'var(--text-muted)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {invites.map((inv, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 160px 36px', gap: 8, alignItems: 'center' }}>
                    ...
                    <button onClick={() => removeInvite(i)} style={{ height: 38, width: 36, borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                      <span className="material-symbols-outlined" style={{ fontSize: 16 }}>close</span>
                    </button>
                  </div>
                ))}
              </div>
              <button onClick={addInvite} style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, border: '1px dashed var(--teal)', background: 'rgba(40,102,110,0.1)', color: 'var(--teal)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                <span className="material-symbols-outlined" style={{ fontSize: 16 }}>add</span>
                Add Another User
              </button>
              <div style={{ marginTop: 16, padding: '12px 14px', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12, color: 'var(--text-secondary)' }}>
          ...
          {/* ── Step 4: Confirmation ── */}
          {step === 4 && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '40px 36px', textAlign: 'center', boxShadow: 'var(--shadow-md)' }}>
              <div style={{ width: 60, height: 60, borderRadius: '50%', background: 'rgba(40,102,110,0.1)', border: '2px solid rgba(40,102,110,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
                <span className="material-symbols-outlined" style={{ fontSize: 28, color: 'var(--teal)' }}>check_circle</span>
              </div>
              <h2 style={{ fontSize: '1.6rem', marginBottom: 8 }}>Workspace Ready</h2>
              <p style={{ color: 'var(--text-primary)', fontSize: 14, marginBottom: 32, lineHeight: 1.7 }}>
                <strong>{org.name}</strong> has been set up. Your invited users can now log in using their email addresses at the respective portals below.
              </p>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 32, textAlign: 'left' }}>
                {invites.filter(i => i.email.trim()).map((inv, i) => {
                  const role = roles.find(r => r.key === inv.roleKey)
                  const portal = inv.roleKey === 'chro' ? 'CHRO Portal' : inv.roleKey === 'hiring-assistant' ? 'Hiring Assistant' : 'Faculty Portal'
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', border: '1px solid var(--border)', borderRadius: 10, background: 'var(--bg)' }}>
                      <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--teal)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                        {(inv.name || inv.email).slice(0, 2).toUpperCase()}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{inv.name || inv.email}</div>
                        <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{inv.email} · {role?.label}</div>
                      </div>
                      <div style={{ fontSize: 11, color: 'var(--teal)', fontWeight: 600 }}>{portal}</div>
                    </div>
                  )
                })}
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
                <a href="http://localhost:5173/login" className="btn-primary">Open HIRIS Platform</a>
              </div>
            </div>
          )}
        </div>
...
function StepCard({ title, desc, children }) {
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '36px 36px 28px', boxShadow: 'var(--shadow-sm)' }}>
      ...
      <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 28, lineHeight: 1.6 }}>{desc}</p>
...
function FormField({ label, required, children }) {
  return (
    <div>
      <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>
